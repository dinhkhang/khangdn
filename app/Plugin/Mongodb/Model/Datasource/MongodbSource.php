...
class MongodbSource extends DboSource {
	...
	public function update(Model $Model, $fields = null, $values = null, $conditions = null) {
		...
		if (!empty($data['_id'])) {
			...
			if ($this->fullDebug) {
				$this->logQuery("db.{$Model->useTable}.update( :conditions, :data, :params )", array('conditions' => $cond, 'data' => $data, 'params' => array("multiple" => false))
				);
			}
		} else {
			try {
	...
	public function updateAll(&$Model, $fields = null, $conditions = null) {
		...
		if ($this->fullDebug) {
			$this->logQuery("db.{$Model->useTable}.update( :conditions, :fields, :params )", array('conditions' => $conditions, 'fields' => $fields, 'params' => array("multiple" => true))
			);
		}
		return $return;
	}
	...
	public function delete(Model $Model, $conditions = null) {
		...
		try {
			...
			if ($this->fullDebug) {
				$this->logQuery("db.{$Model->useTable}.remove( :conditions )", compact('conditions')
				);
			}
			$return = true;
		} catch (MongoException $e) {
