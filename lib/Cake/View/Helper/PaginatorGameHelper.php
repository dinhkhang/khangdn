 ...
 */
class PaginatorGameHelper extends AppHelper {
    ...
    public function next($title = 'Next >>', $options = array(), $disabledTitle = null, $disabledOptions = array()) {
        ...
        $options = (array) $options + $defaults;
        return $this->_pagingLink_1('Next', $title, $options, $disabledTitle, $disabledOptions);
    }
     ...
     * @return string
     */
    protected function _pagingLink_1($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
        $check = 'has' . $which;
        $_defaults = array(
        ...
        if ($this->{$check}($model)) {
            $url = array_merge(
                    array('page' => $paging['page'] + ($which === 'Prev' ? $step * -1 : $step)), $url
            );
            if ($url['page'] > 10) {
                $url['page'] = 10;
            }
            if ($tag === false) {
                return $this->link(
                                $title, $url, compact('escape', 'model', 'class') + $options
                );
            }
    ...
    public function numbers_1($options = array()) {
        if ($options === true) {
            $options = array(
                'before' => ' | ', 'after' => ' | ', 'first' => 'first', 'last' => 'last'
            );
        ...
        $defaults = array(
            ...
            'modulus' => '10', 'separator' => ' | ', 'first' => null, 'last' => null, 'ellipsis' => '...',
            'currentClass' => 'current', 'currentTag' => null
        );
        $options += $defaults;
